...
from gradient_boosting import bands_mean, pipe
...
logging.basicConfig(filename=os.path.join(root_path, 'logs', 'evaluation_gb.log'), filemode='a', level=logging.INFO,
...
def main(options):
    # Load Spectral Signatures, Spectral Indices 
    ...
    cl_path = os.path.join(up(os.path.abspath(__file__)), 'gb_classifier.joblib')
    dump(pipe, cl_path)
    print("Classifier is saved at: " + str(cl_path))
    logging.info("Classifier is saved at: " + str(cl_path))

    print('\t\t Gradient Boosting Results on ' + options['eval_set'] + ' Set')
    conf_mat = confusion_matrix(y_test, pipe.predict(X_test), pipe.classes_)
    ...
    if options['predict_masks']:
        ...
        for roi in tqdm(ROIs):
            ...
            output_image = os.path.join(options['gen_masks_path'],
                                        os.path.basename(roi_file).split('.tif')[0] + '_rf.tif')
            ...
            image_features = np.concatenate([image_features, si_image_features], axis=1)

            # Write it
            with rasterio.open(output_image, 'w', **meta) as dst:
                # use classifier to predict labels for the whole image
                predictions = pipe.predict(image_features)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--eval_set', default='test', type=str,
                        help="Set for the evaluation 'val' or 'test' for final testing")
    parser.add_argument('--predict_masks', default=True, type=bool, help='Generate test set prediction masks?')
    parser.add_argument('--gen_masks_path', default=os.path.join(root_path, 'data', 'predicted_gb'),
                        help='Path to where to produce store predictions')

    parser.add_argument('--agg_to_water', default='["Mixed Water", "Wakes", "Cloud Shadows", "Waves"]', type=str,
                        help='Specify the Classes that will aggregate with Marine Water')
